...
// figure out which GA to use 
	...
	Widget toplevel = XtAppInitialize(&appc, "TSPView", (XrmOptionDescRec *) NULL, 0,
	...
	thegc = XCreateGC(XtDisplay(toplevel),
			  RootWindowOfScreen(XtScreen(toplevel)), valueMask, &gcValues);
...
void DrawIndividual(GAGenome & g, Display * display, Drawable drawable, GC gc, int xx, int yy)
{
...
{
	Widget shell = XtVaCreatePopupShell("shell", topLevelShellWidgetClass, toplevel, NULL);
	...
	Pixel fg, bg;
	unsigned int depth;
	XtVaGetValues(form, XmNforeground, &fg, XmNbackground, &bg, XmNdepth, &depth, NULL);
	pix =
