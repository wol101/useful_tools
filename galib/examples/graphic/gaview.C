...
// Create the appropriate genome and genetic algorithm
	...
	if (theAppData.whichGenome == 1)
	{
		...
		map.add(31, minx[theAppData.whichFunction], maxx[theAppData.whichFunction]);
		map.add(31, minx[theAppData.whichFunction], maxx[theAppData.whichFunction]);
...
{
	if ((n < 0 && theAppData.ga->done() == gaFalse) || theAppData.ga->generation() < n)
	{
		theAppData.ga->step();
...
{
	AppDataPtr data = (AppDataPtr) cd;
	data->procid = XtAppAddWorkProc(data->appc, (XtWorkProc) Evolve, (XtPointer) (-1));
}
...
void DrawPopulation(Widget widget, const GAPopulation & pop, GC dotgc, GC bestgc)
{
	...
	if (theAppData.whichGenome == 1)
	{
		...
		xbest = originx + d / 2 + factor * ((GABin2DecGenome &) (pop.best())).phenotype(0);
		ybest = originy + d / 2 - factor * ((GABin2DecGenome &) (pop.best())).phenotype(1);
	} else
	{
		for (int i = 0; i < pop.size(); i++)
		{
		...
		xbest = originx + d / 2 + factor * ((GARealGenome &) (pop.best())).gene(0);
		ybest = originy + d / 2 - factor * ((GARealGenome &) (pop.best())).gene(1);
	}

	XDrawPoints(XtDisplay(widget), XtWindow(widget), dotgc, pts, npts, CoordModeOrigin);
...
float Bin2DecObjective(GAGenome & g)
{
	...
	return (obj[theAppData.whichFunction]) (genome.phenotype(0), genome.phenotype(1));
}
...
{
	float z =
		-((x * x + y - 11) * (x * x + y - 11) + (x + y * y - 7) * (x + y * y - 7)) / 200 +
		10;
	return z;
}
...
{
	Pixmap icon = XCreateBitmapFromData(XtDisplay(toplevel),
					    RootWindowOfScreen(XtScreen(toplevel)),
					    (char *) gaview_bits, gaview_width, gaview_height);
	Pixmap mask = XCreateBitmapFromData(XtDisplay(toplevel),
					    RootWindowOfScreen(XtScreen(toplevel)),
					    (char *) gaview_bits, gaview_width, gaview_height);
	Widget shell = XtVaCreatePopupShell("shell", topLevelShellWidgetClass, toplevel,
					    XmNiconPixmap, icon,
					    XmNiconMask, mask,
					    NULL);
	Widget form = XtVaCreateManagedWidget("form", xmFormWidgetClass, shell, NULL);

	Pixmap pix;
	Pixel fg, bg;
	unsigned int depth;
	XtVaGetValues(form, XtNforeground, &fg, XtNbackground, &bg, XtNdepth, &depth, NULL);

	pix =
		XCreatePixmapFromBitmapData(XtDisplay(form),
					    RootWindowOfScreen(XtScreen(form)),
					    (char *) bm[bmRewind].bits,
					    bm[bmRewind].width, bm[bmRewind].height, fg, bg, depth);
	Widget rewind = XtVaCreateManagedWidget("rewind", xmPushButtonWidgetClass, form,
	...
	pix =
		XCreatePixmapFromBitmapData(XtDisplay(form),
					    RootWindowOfScreen(XtScreen(form)),
					    (char *) bm[bmStop].bits,
					    bm[bmStop].width, bm[bmStop].height, fg, bg, depth);
	Widget stop = XtVaCreateManagedWidget("stop", xmPushButtonWidgetClass, form,
	...
	Widget step = XtVaCreateManagedWidget("step", xmPushButtonWidgetClass, form,
					      XmNleftAttachment, XmATTACH_WIDGET,
					      XmNleftWidget, stop,
					      XmNbottomAttachment, XmATTACH_FORM,
					      XmNlabelType, XmPIXMAP,
	...
	Widget some = XtVaCreateManagedWidget("some", xmPushButtonWidgetClass, form,
					      XmNleftAttachment, XmATTACH_WIDGET,
					      XmNleftWidget, step,
					      XmNbottomAttachment, XmATTACH_FORM,
					      XmNlabelType, XmPIXMAP,
	...
	Widget evolve = XtVaCreateManagedWidget("evolve", xmPushButtonWidgetClass, form,
						XmNleftAttachment, XmATTACH_WIDGET,
						XmNleftWidget, some,
						XmNbottomAttachment, XmATTACH_FORM,
						XmNlabelType, XmPIXMAP,
	...
	XtAddCallback(some, XmNactivateCallback, EvolveSomeCB, (XtPointer) & theAppData);
	XtAddCallback(evolve, XmNactivateCallback, EvolveCB, (XtPointer) & theAppData);

	Widget params = XtVaCreateManagedWidget("params", xmPushButtonWidgetClass, form,
						XmNrightAttachment, XmATTACH_FORM,
						XmNbottomAttachment, XmATTACH_FORM,
						NULL);
	Widget stats = XtVaCreateManagedWidget("stats", xmPushButtonWidgetClass, form,
					       XmNrightAttachment, XmATTACH_WIDGET,
					       XmNrightWidget, params,
					       XmNbottomAttachment, XmATTACH_FORM,
					       NULL);
	Widget score = XtVaCreateManagedWidget("score", xmPushButtonWidgetClass, form,
					       XmNrightAttachment, XmATTACH_WIDGET,
					       XmNrightWidget, stats,
					       XmNbottomAttachment, XmATTACH_FORM,
					       NULL);

	XtAddCallback(params, XmNactivateCallback, DumpParamsCB, (XtPointer) theAppData.ga);
	XtAddCallback(stats, XmNactivateCallback, DumpStatsCB, (XtPointer) theAppData.ga);
	XtAddCallback(score, XmNactivateCallback, DumpScoreCB, (XtPointer) theAppData.ga);
	...
	Widget frame = XtVaCreateManagedWidget("frame", xmFrameWidgetClass, form,
					       ...
					       NULL);
	theAppData.canvas =
		XtVaCreateManagedWidget("canvas", xmDrawingAreaWidgetClass, frame, NULL);

	XtAddCallback(theAppData.canvas, XmNexposeCallback, DrawCB, (XtPointer) & theAppData);
...
Widget ConstructWidgets(Widget toplevel)
{
	...
	theAppData.counter = XtVaCreateManagedWidget("counter", labelWidgetClass, form, NULL);

	theAppData.canvas =
		XtVaCreateManagedWidget("canvas", widgetClass, form,
					XtNfromVert, theAppData.counter, NULL);
	...
	Widget ctrlbox = XtVaCreateManagedWidget("controls", boxWidgetClass, form,
						 ...
						 NULL);
	Widget rewind = XtVaCreateManagedWidget("rewind", commandWidgetClass, ctrlbox, NULL);
	Widget stop = XtVaCreateManagedWidget("stop", commandWidgetClass, ctrlbox, NULL);
	Widget step = XtVaCreateManagedWidget("step", commandWidgetClass, ctrlbox, NULL);
	Widget some = XtVaCreateManagedWidget("some", commandWidgetClass, ctrlbox, NULL);
	Widget evolve = XtVaCreateManagedWidget("evolve", commandWidgetClass, ctrlbox, NULL);
	...
	Widget params = XtVaCreateManagedWidget("params", commandWidgetClass, ctrlbox, NULL);
	Widget stats = XtVaCreateManagedWidget("stats", commandWidgetClass, ctrlbox, NULL);
	Widget score = XtVaCreateManagedWidget("score", commandWidgetClass, ctrlbox, NULL);
	...
	Widget quit = XtVaCreateManagedWidget("quit", commandWidgetClass, ctrlbox, NULL);
