...
float GenomeEvaluator(GAGenome & g)
{
	GA1DBinaryStringGenome & genome = (GA1DBinaryStringGenome &) g;
#ifdef DEBUG
	char buf[255];
...
int SendGenomeInitialize(GAGenome & g, int tid)
{
	GA1DBinaryStringGenome & genome = (GA1DBinaryStringGenome &) g;
	int status = 0;
...
// initialization.
int RecvGenomeInitialize(GAGenome & g)
{
	GA1DBinaryStringGenome & genome = (GA1DBinaryStringGenome &) g;
	int status = 0;
	int length = 1;
...
int SendGenomeData(GAGenome & g, int tid)
{
	GA1DBinaryStringGenome & genome = (GA1DBinaryStringGenome &) g;
	static int *bits = 0;
	static int nbits = 0;
...
int SendGenomeScore(GAGenome & g, int tid)
{
	int status = 0;
	float score = g.score();
	status = pvm_initsend(PvmDataDefault);
...
int RecvGenomeScore(GAGenome & g)
{
	int status = 0;
	float score = 0.0;
...
// configured, initialized genome.
		if (next < pop.size() && (bufid = pvm_nrecv(-1, MSG_READY)) != 0)
		{
			if (bufid > 0)
			{
				...
				{
					cerr << "PopInit: error sending initialize command to: " <<
						tid;
					cerr << "  genome " << next <<
						" will be inited by next slave\n";
...
// the slave and stuff it into the appropriate genome in the population.
			...
			if (bufid > 0)
			{
				...
				if ((who = id2idx(tid, *data)) >= 0)
				{
					if (index[who] >= 0)
					{
						...
						{
							cerr <<
								"PopInit: error receiving data from: "
								<< tid;
							cerr << "  error code is: " << status <<
								"\n";
						}
		...
		if (next == pop.size() && outstanding == 0)
			done = 1;
		if (next > pop.size())
		{
		...
		if (outstanding > 0 && (bufid = pvm_nrecv(-1, MSG_GENOME_SCORE)) != 0)
		{
			if (bufid > 0)
			{
				...
				if ((who = id2idx(tid, *data)) >= 0)
				{
					...
					{
						status =
							RecvGenomeScore(pop.individual(index[who]));
						if (status >= 0)
						{
							index[who] = -1;
							outstanding--;
						...
						{
							cerr <<
								"PopEval: error receiving score from: "
								<< tid;
							cerr << "  error code is: " << status <<
								"\n";
						}
