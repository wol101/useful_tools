...
class BitString { friend class BitSubString;
	friend class BitPattern;
      protected:
	 BitStrRep * rep;
...
// iterators
	...
	int previous(int pos, unsigned int b = 1) const { return prev(pos, b);
	}					 /* Obsolete synonym */
// searching & matchingulationred inline belown, MA 02111-1307, USA.
...
// other friends & utilities
	...
	friend BitString common_prefix(const BitString &, const BitString &, int pos = 0);
	friend BitString common_suffix(const BitString &, const BitString &, int pos = -1);
...
// indirect friends
	...
	friend BitPattern atoBitPattern(const char *s, char f = '0', char t = '1', char x = 'X');
...
class BitPattern { public:
	...
	friend const char *BitPatterntoa(const BitPattern & p, char f /*='0'*/ , char t /*='1'*/ , char x
								     /*='X'*/
		);
...
inline BitString::
BitString(const BitSubString & y):rep(BStr_alloc(0, y.S.rep->s, y.pos, y.pos + y.len, y.len))
{
}

inline BitString::~BitString()
{
	if (rep != &_nilBitStrRep)
		delete rep;
}
...
inline BitSubString::BitSubString(const BitSubString & x):S(x.S), pos(x.pos), len(x.len)
{
}

inline BitSubString::BitSubString(BitString & x, int p, int l):S(x), pos(p), len(l)
{
}
...
inline BitPattern::BitPattern(const BitString & p, const BitString & m):pattern(p), mask(m)
{
}

inline BitPattern::BitPattern(const BitPattern & b):pattern(b.pattern), mask(b.mask)
{
}
...
inline BitStrBit::operator unsigned int ()
const { return src.test(pos);
