...
{
	_BS_word mask = (ONES _BS_LEFT(_BS_BITS_PER_WORD - length)) _BS_RIGHT dstbit;
	_BS_word word0 = *psrc++;
...
if (psrc >= pdst)				 /* Do the updates in forward direction. */
{
	...
	for (; length >= _BS_BITS_PER_WORD; length -= _BS_BITS_PER_WORD)
	{
		...
		DOIT_SOLID(*pdst, (word0 _BS_LEFT shift0) | (word1 _BS_RIGHT shift1));
		pdst++;
		word0 = word1;
	}
	if (length > 0)
...
} else						 /* Do the updates in backward direction. */
{
	...
	if (shift0 < 0)
	{
		...
		shift1 = -shift0;
		shift0 += _BS_BITS_PER_WORD;
		DOIT(*pdst, (word0 _BS_RIGHT shift1) | (word1 _BS_LEFT shift0), mask);
		word0 = word1;
	} else
	{
		shift1 = _BS_BITS_PER_WORD - shift0;
	...
	{
		register _BS_word word1 = *psrc--;
		DOIT_SOLID(*pdst, (word0 _BS_RIGHT shift1) | (word1 _BS_LEFT shift0));
		pdst--;
		word0 = word1;
	}
	if (length > 0)
