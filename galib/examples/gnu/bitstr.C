...
BitStringGenome::BitStringGenome(unsigned int l, GAGenome::Evaluator f,
				 void *u):GAGenome(UniformInitializer, UniformMutator, Comparator),
BitString()
{
...
GAGenome *BitStringGenome::clone(GAGenome::CloneMethod) const
{
...
void BitStringGenome::UniformInitializer(GAGenome & c)
{
...
int BitStringGenome::UniformMutator(GAGenome & c, float pmut)
{
	BitStringGenome & genome = (BitStringGenome &) c;
	register int n, i;
	if (pmut <= 0.0)
...
float BitStringGenome::Comparator(const GAGenome & a, const GAGenome & b)
{
