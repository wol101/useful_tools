...
PVMDemeGA & PVMDemeGA::operator = (const PVMDemeGA & orig)
{
	if (&orig != this)
		copy(orig);
	return *this;
}
void PVMDemeGA::copy(const GAGeneticAlgorithm & g)
{
...
void PVMDemeGA::initialize(unsigned int seed)
{
	GARandomSeed(seed);
...
int PVMDemeGA::spawn(const char *slavename)
{
	_mid = pvm_mytid();
	if (_mid < 0)
	{
...
int PVMDemeGA::tid2idx(int taskid) const
{
	int idx = -1;
