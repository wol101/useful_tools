...
// GAlib objects).
class RobotPathGenome:public GAGenome { public:
	...
	static int Crossover(const GAGenome &, const GAGenome &, GAGenome *, GAGenome *);
	 ...
	 RobotPathGenome(const RobotPathGenome & orig) {
		...
		list = 0;
		copy(orig);
	} RobotPathGenome operator = (const GAGenome & arg) { copy(arg);
		return *this;
...
RobotPathGenome::RobotPathGenome(int nrobots, int pathlength):GAGenome(Initializer, Mutator,
								       Comparator)
{
	evaluator(Evaluator);
	crossover(Crossover);
...
int RobotPathGenome::Crossover(const GAGenome & a, const GAGenome & b, GAGenome * c, GAGenome * d)
{
	...
	if (c && d)
	{
		...
		for (int i = 0; i < mom.npaths(); i++)
			GAListGenome < int >::PartialMatchCrossover(mom.path(i),
								    dad.path(i),
