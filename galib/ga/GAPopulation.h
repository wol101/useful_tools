// $Header: /home/wis/Unix/cvsroot/galib/ga/GAPopulation.h,v 1.1.1.1 2001/05/11 16:00:10 ws Exp $
/* ----------------------------------------------------------------------------
  population.h
  mbwall 3aug94
...
class GAPopulation:public GAID {
	...
	void touch()
	{
	   rsorted = ssorted = selectready = divved = statted = scaled = evaluated =
	   gaFalse;} void statistics(GABoolean flag = gaFalse) const;
	...
	void sort(GABoolean flag = gaFalse, SortBasis basis = RAW) const;
		...
		float sum() const { if (!statted)
			statistics();
		return rawSum;
		} float ave() const { if (!statted)
			statistics();
		return rawAve;
		} float var() const { if (!statted)
			statistics();
		return rawVar;
		} float dev() const { if (!statted)
			statistics();
		return rawDev;
		} float max() const { if (!statted)
			statistics();
		return rawMax;
		} float min() const { if (!statted)
			statistics();
		return rawMin;
		} float div() const { if (!divved)
			diversity();
		return popDiv;
		} float div(unsigned int i, unsigned int j) const { if (!divved)
			diversity();
		return indDiv[i * n + j];
		} float fitsum() const { if (!scaled)
			scale();
		return fitSum;
		} float fitave() const { if (!scaled)
			scale();
		return fitAve;
		} float fitmax() const { if (!scaled)
			scale();
		return fitMax;
		} float fitmin() const { if (!scaled)
			scale();
		return fitMin;
		} float fitvar() const { if (!scaled)
			scale();
		return fitVar;
		} float fitdev() const { if (!scaled)
			scale();
		return fitDev;
		...
		if (evaluated == gaFalse || flag == gaTrue)
		{
		 ...
		 GAGenome & select()
		 {
		 if (!selectready) prepselect(); return slct->select();}
		 ...
		 This->scaled = gaFalse; return *sclscm;}
		 GAScalingScheme & scaling(const GAScalingScheme &);
		 GAGeneticAlgorithm * geneticAlgorithm()const
		 {
		 return ga;}
		 GAGeneticAlgorithm * geneticAlgorithm(GAGeneticAlgorithm &); void *userData()const
		 {
		 return ud;}
		 ...
		 sort(gaFalse, basis); return ((basis == RAW) ? *(rind[i]) : *(sind[i]));}
		 ...
		 GAGenome * replace(GAGenome *, int which = RANDOM, SortBasis basis =
				    RAW);
