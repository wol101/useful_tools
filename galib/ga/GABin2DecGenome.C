// $Header: /home/wis/Unix/cvsroot/galib/ga/GABin2DecGenome.C,v 1.1.1.1 2001/05/11 16:00:07 ws Exp $
/* ----------------------------------------------------------------------------
  bin2dec.C
  mbwall 19apr95
...
int GABin2DecPhenotype::equal(const GABin2DecPhenotype & b) const
{
	...
	if (memcmp(core->nbits, b.core->nbits, core->n * sizeof(unsigned short))
	    != 0
	    || memcmp(core->oset, b.core->oset,
		      core->n * sizeof(unsigned short)) != 0
...
int GABin2DecGenome::read(istream & is)
{
	float value;
...
int GABin2DecGenome::write(ostream & os) const
{
	for (unsigned int i = 0; i < phenotypes().nPhenotypes(); i++)
		os << phenotype(i) << " ";
	return 0;
