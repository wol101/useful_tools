// $Header: /home/wis/Unix/cvsroot/galib/ga/GA2DArrayGenome.C,v 1.1.1.1 2001/05/11 16:00:04 ws Exp $
/* ----------------------------------------------------------------------------
  array2.C
  mbwall 25feb95
...
template < class T >
	GA2DArrayGenome < T >::GA2DArrayGenome(unsigned int width,
unsigned int height, GAGenome::Evaluator f, void *u):
GAArray < T > (width * height), GAGenome(DEFAULT_2DARRAY_INITIALIZER,
					 DEFAULT_2DARRAY_MUTATOR, DEFAULT_2DARRAY_COMPARATOR)
{
	evaluator(f);
	userData(u);
...
template < class T >
	GA2DArrayGenome < T >::GA2DArrayGenome(const GA2DArrayGenome < T > &orig):GAArray < T >
	(orig.sz)
{
...
template < class T > GAGenome * GA2DArrayGenome < T >::clone(GAGenome::CloneMethod flag) const
{
...
#ifndef NO_STREAMS
template < class T > int GA2DArrayGenome < T >::read(istream &)
{
...
template < class T > int GA2DArrayGenome < T >::write(ostream & os) const
{
...
template < class T > int GA2DArrayGenome < T >::resizeBehaviour(GAGenome::Dimension which) const
{
	int val = 0;
	if (which == WIDTH)
	{
...
template < class T > int
	GA2DArrayGenome < T >::resizeBehaviour(GAGenome::Dimension which,
...
template < class T >
	GA2DArrayAlleleGenome < T >::GA2DArrayAlleleGenome(unsigned int width,
							   ...
							   void *u):GA2DArrayGenome < T > (width,
											   height,
											   f, u)
{
	naset = sa.size();
...
template < class T >
	...
	T >::GA2DArrayAlleleGenome(const GA2DArrayAlleleGenome < T > &orig):GA2DArrayGenome < T >
	(orig.nx, orig.ny)
{
	naset = 0;
...
template < class T > GAGenome * GA2DArrayAlleleGenome < T >::clone(GAGenome::CloneMethod) const
{
...
#ifndef NO_STREAMS
template < class T > int GA2DArrayAlleleGenome < T >::read(istream & is)
{
	return GA2DArrayGenome < T >::read(is);
}

template < class T > int GA2DArrayAlleleGenome < T >::write(ostream & os) const
{
...
template < class ARRAY_TYPE > float
	GA2DArrayGenome < ARRAY_TYPE >::ElementComparator(const GAGenome & a, const GAGenome & b)
{
...
template < class T > int
	GA2DArrayGenome < T >::UniformCrossover(const GAGenome & p1,
...
template < class T > int
	GA2DArrayGenome < T >::OnePointCrossover(const GAGenome & p1,
						 const GAGenome & p2, GAGenome * c1, GAGenome * c2)
{
	const GA2DArrayGenome < T > &mom = DYN_CAST(const GA2DArrayGenome < T > &, p1);
	const GA2DArrayGenome < T > &dad = DYN_CAST(const GA2DArrayGenome < T > &, p2);
...
template < class T > int
	GA2DArrayGenome < T >::EvenOddCrossover(const GAGenome & p1,
						const GAGenome & p2, GAGenome * c1, GAGenome * c2)
{
	const GA2DArrayGenome < T > &mom = DYN_CAST(const GA2DArrayGenome < T > &, p1);
	const GA2DArrayGenome < T > &dad = DYN_CAST(const GA2DArrayGenome < T > &, p2);
