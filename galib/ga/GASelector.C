// $Header: /home/wis/Unix/cvsroot/galib/ga/GASelector.C,v 1.1.1.1 2001/05/11 16:00:09 ws Exp $
/* ----------------------------------------------------------------------------
  selector.C
  mbwall 10aug94
				 ...
				 (pop->fitmax() <= 0 && pop->fitmin() <= 0))
		{
			...
			for (i = 0; i < pop->size(); i++)
			{
				if (pop->order() == GAPopulation::HIGH_IS_BEST)
					...
					expected =
						(-pop->individual(i, GAPopulation::SCALED).
						 fitness() + pop->fitmax() +
						 pop->fitmin()) / pop->fitave();
				ne = (int) expected;
				fraction[i] = expected - ne;
				while (ne > 0)
				{
