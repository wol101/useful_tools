// $Header: /home/wis/Unix/cvsroot/galib/ga/GAStatistics.C,v 1.1.1.1 2001/05/11 16:00:07 ws Exp $
/* ----------------------------------------------------------------------------
  statistics.C
  mbwall 28jul94
...
void GAStatistics::updateBestIndividual(const GAPopulation & pop, GABoolean flag)
{
	...
	} else
	{
		...
		if (boa->order() == GAPopulation::HIGH_IS_BEST)
		{
			...
			{
				for (k = 0;
				     pop.best(i).score() < boa->best(k).score()
				     && k < boa->size(); k++);
...
#ifndef NO_STREAMS
	ofstream
		outfile(scorefile, ((gen[0] == 0) ?
				    (ios::out | ios::trunc) : (ios::out | ios::app)));
...
int GAStatistics::write(ostream & os) const
{
...
int GAStatistics::scores(ostream & os, int w)
{
	if (w == NoScores)
		w = which;
