// $Header: /home/wis/Unix/cvsroot/galib/ga/GARealGenome.C,v 1.1.1.1 2001/05/11 16:00:05 ws Exp $
/* ----------------------------------------------------------------------------
  real.C
  mbwall 11nov95
...
// wis - had to add lots of (int) casts to GAAllele::type values to get this to work in Metrowerks 3 - bizarre and strange
...
GA1DArrayAlleleGenome < float >::GA1DArrayAlleleGenome(unsigned int length,
						       ...
						       void *u):GA1DArrayGenome < float >(length, f,
											  u)
{
	naset = 1;
...
GA1DArrayAlleleGenome <
	float >::GA1DArrayAlleleGenome(const GAAlleleSetArray < float >&sa,
				       GAGenome::Evaluator f,
				       void *u):GA1DArrayGenome < float >(sa.size(), f, u)
{
	naset = sa.size();
...
int GA1DArrayAlleleGenome < float >::read(istream & is)
{
	unsigned int i = 0;
	float val;
	do
...
float GAAlleleSet < float >::allele() const
{
	...
	else if (core->type == (int) GAAllele::DISCRETIZED)
	{
		float n = (core->a[1] - core->a[0]) / core->a[2];
		int m = (int) n;
		if (core->lowerb == (int) GAAllele::EXCLUSIVE)
			m -= 1;
		if (core->upperb == (int) GAAllele::EXCLUSIVE)
			m -= 1;
	...
	} else
	{
		if (core->a[0] == core->a[1] &&
		    core->lowerb == (int) GAAllele::EXCLUSIVE &&
		    core->upperb == (int) GAAllele::EXCLUSIVE)
		{
			value = core->a[0];
		} else
		{
...
// the same...
int
GARealArithmeticCrossover(const GAGenome & p1, const GAGenome & p2, GAGenome * c1, GAGenome * c2)
{
	...
	{
		GA1DArrayGenome < float >&sis = (c1 ? DYN_CAST(GA1DArrayGenome < float >&,
							       *c1) :
						 DYN_CAST(GA1DArrayGenome < float >&,
							  *c2));
