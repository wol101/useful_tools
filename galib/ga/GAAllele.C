// $Header: /home/wis/Unix/cvsroot/galib/ga/GAAllele.C,v 1.1.1.1 2001/05/11 16:00:09 ws Exp $
/* ----------------------------------------------------------------------------
  allele.C
  mbwall 21mar95
...
template < class T > GAAlleleSetCore < T >::GAAlleleSetCore(unsigned int n, const T array[]):
...
template < class T >
	GAAlleleSetCore < T >::GAAlleleSetCore(const T & lower, const T & upper,
					       ...
					       GAAllele::BoundType ub):type(GAAllele::BOUNDED),
csz(GA_ALLELE_CHUNK), sz(2), SZ(2), a(new T[2])
{
	a[0] = lower;
	a[1] = upper;
	lowerb = lb;
...
template < class T >
	GAAlleleSetCore < T >::GAAlleleSetCore(const T & lower, const T & upper,
					       const T & increment,
					       GAAllele::BoundType lb,
					       GAAllele::BoundType ub):type(GAAllele::DISCRETIZED),
csz(GA_ALLELE_CHUNK), sz(3), SZ(3), a(new T[3])
{
	a[0] = lower;
	a[1] = upper;
	a[2] = increment;
...
template < class T >
	GAAlleleSetCore < T >::GAAlleleSetCore(const GAAlleleSetCore < T > &orig):csz(orig.csz),
sz(orig.sz), SZ(orig.SZ), a(new T[orig.SZ])
{
...
template < class T > GAAlleleSetCore < T > &GAAlleleSetCore < T >::operator =
	(const GAAlleleSetCore < T > &orig)
{
	if (this == &orig)
		return *this;
...
template < class T > int GAAlleleSet < T >::add(const T & alle)
{
	...
	if (core->type != (int) GAAllele::ENUMERATED)
		return 1;
	if (core->sz >= core->SZ)
	{
		core->SZ += core->csz;
...
#ifndef NO_STREAMS
template < class T > int GAAlleleSet < T >::read(istream &)
{
	GAErr(GA_LOC, "GAAlleleSet", "read", gaErrOpUndef);
	return 1;
}
template < class T > int GAAlleleSet < T >::write(ostream &) const
{
...
template < class T > GAAlleleSetArray < T >::GAAlleleSetArray(const GAAlleleSet < T > &s):
...
template < class T >
	GAAlleleSetArray < T >::GAAlleleSetArray(const GAAlleleSetArray < T >
						 &orig):csz(orig.csz), sz(orig.sz), SZ(orig.SZ),
...
template < class T > GAAlleleSetArray < T > &GAAlleleSetArray <
	T >::operator = (const GAAlleleSetArray < T > &orig)
{
	if (this == &orig)
		return *this;
	unsigned int i;
