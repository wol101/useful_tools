// $Header: /home/wis/Unix/cvsroot/galib/ga/GABaseGA.C,v 1.1.1.1 2001/05/11 16:00:08 ws Exp $
/* ----------------------------------------------------------------------------
  gabase.C
  mbwall 28jul94
...
GAGeneticAlgorithm::GAGeneticAlgorithm(const GAGeneticAlgorithm & ga):stats(ga.stats),
params(ga.params)
{
	pop = ga.pop->clone();
	pop->geneticAlgorithm(*this);
...
const GAParameterList & GAGeneticAlgorithm::parameters(istream & is, GABoolean flag)
{
	params.read(is, flag);
	for (int i = 0; i < params.size(); i++)
		setptr(params[i].fullname(), params[i].value());
