// $Header: /home/wis/Unix/cvsroot/galib/ga/GABin2DecGenome.h,v 1.1.1.1 2001/05/11 16:00:07 ws Exp $
/* ----------------------------------------------------------------------------
  bin2dec.h
  mbwall 19apr95
...
class GABin2DecPhenotypeCore { public:
...
class GABin2DecPhenotype { public:
	GABin2DecPhenotype():core(new GABin2DecPhenotypeCore) {
	}
		GABin2DecPhenotype(const GABin2DecPhenotype & p):core(new
								      GABin2DecPhenotypeCore
								      (*(p.core))) {
	}
	...
	} void link(GABin2DecPhenotype & p) { core->cnt -= 1;
		if (core->cnt == 0)
			delete core;
		 core = p.core;
		 core->cnt += 1;
...
class GABin2DecGenome:public GA1DBinaryStringGenome {
	...
	GABin2DecGenome(const GABin2DecPhenotype & p,
			...
			void *u =
			(void *)0):GA1DBinaryStringGenome(p.size(), f, u),
	...
	void encoder(GABinaryEncoder e) { encode = e;
		_evaluated = gaFalse;
	} void decoder(GABinaryDecoder d) { decode = d;
		_evaluated = gaFalse;
      } protected:
