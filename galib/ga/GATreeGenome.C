// $Header: /home/wis/Unix/cvsroot/galib/ga/GATreeGenome.C,v 1.1.1.1 2001/05/11 16:00:05 ws Exp $
/* ----------------------------------------------------------------------------
  tree.C
  mbwall 25feb95
...
template < class T > GATreeGenome < T >::GATreeGenome(GAGenome::Evaluator f, void *u):
...
template < class T >
	GATreeGenome < T >::GATreeGenome(const GATreeGenome < T > &orig):GATree < T > (), GAGenome()
{
...
template < class T > GAGenome * GATreeGenome < T >::clone(GAGenome::CloneMethod flag) const
{
...
template < class T > void _tt(ostream & os, GANode < T > *n)
{
	if (!n)
		return;
...
template < class T > int GATreeGenome < T >::write(ostream & os) const
{
...
template < class T > int GATreeGenome < T >::DestructiveMutator(GAGenome & c, float pmut)
{
...
template < class T > int GATreeGenome < T >::SwapSubtreeMutator(GAGenome & c, float pmut)
{
	GATreeGenome < T > &child = DYN_CAST(GATreeGenome < T > &, c);
	register int n, i;
	int a, b;
...
template < class T > int
	GATreeGenome < T >::OnePointCrossover(const GAGenome & p1,
