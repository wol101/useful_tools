// $Header: /home/wis/Unix/cvsroot/galib/ga/GATree.h,v 1.1.1.1 2001/05/11 16:00:05 ws Exp $
/* ----------------------------------------------------------------------------
  treetmpl.h
  mbwall 25feb95
...
// methods that modify the state of the tree
	...
	int insert(GATree < T > *t, GATreeBASE::Location where = GATreeBASE::BELOW) {
		if (this == t)
		{
		 ...
		 {
		 delete c; return GATreeBASE::ERR;}
		 iter.node = c; return GATreeBASE::NO_ERR;}
...
// of the tree.
      ...
      protected:
		 ...
		 return GATreeBASE::ERR; iter.node = n; return GATreeBASE::NO_ERR;}
		 GATreeIter < T > iter; friend class GATreeIter < T >;};
...
---------------------------------------------------------------------------- */
template < class T > class GATree; template < class T > class GATreeIter:public GATreeIterBASE
		 {
      public:
      GATreeIter():GATreeIterBASE()
		 {
		 }
      GATreeIter(const GATree < T > &t):GATreeIterBASE(t)
		 {
		 node = t.iter.node;}
		 ...
		 T * current()
		 {
		 return (node ? &((GANode < T > *)node)->contents : (T *) 0);}
		 T * root()
		 {
		 ...
		 T * eldest()
		 {
		 return (node ? &((GANode < T > *)GATreeIterBASE::eldest(node))->contents : (T *)
			 0);}
		 T * youngest()
		 {
		 return (node ? &((GANode < T > *)GATreeIterBASE::youngest(node))->contents : (T *)
			 0);}
