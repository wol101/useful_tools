// $Header: /home/wis/Unix/cvsroot/galib/ga/GAListGenome.C,v 1.1.1.1 2001/05/11 16:00:05 ws Exp $
/* ----------------------------------------------------------------------------
  list.C
  mbwall 25feb95
...
template < class T > GAListGenome < T >::GAListGenome(GAGenome::Evaluator f, void *u):
...
template < class T >
	GAListGenome < T >::GAListGenome(const GAListGenome < T > &orig):GAList < T > (), GAGenome()
{
...
template < class T > GAGenome * GAListGenome < T >::clone(GAGenome::CloneMethod flag) const
{
...
template < class T > int GAListGenome < T >::write(ostream & os) const
{
	...
	os << hd->prev << " ";
	os.width(10);
	os << &(DYN_CAST(GANode < T > *, hd)->contents) << "\n";
...
template < class T > int GAListGenome < T >::equal(const GAGenome & c) const
{
	if (this == &c)
		return 1;
...
template < class T > int GAListGenome < T >::DestructiveMutator(GAGenome & c, float pmut)
{
...
template < class T > int
	GAListGenome < T >::OnePointCrossover(const GAGenome & p1,
...
template < class T > int
	GAListGenome < T >::PartialMatchCrossover(const GAGenome & p1,
						  const GAGenome & p2, GAGenome * c1, GAGenome * c2)
{
	const GAListGenome < T > &mom = DYN_CAST(const GAListGenome < T > &, p1);
	const GAListGenome < T > &dad = DYN_CAST(const GAListGenome < T > &, p2);
...
template < class T > int
GAListIsHole(const GAListGenome < T > &child,
	     const GAListGenome < T > &parent, int index, int a, int b)
{
...
template < class T > int
 GAListGenome < T >::OrderCrossover(const GAGenome & p1,
				    const GAGenome & p2, GAGenome * c1, GAGenome * c2)
{
	const GAListGenome < T > &mom = DYN_CAST(const GAListGenome < T > &, p1);
	const GAListGenome < T > &dad = DYN_CAST(const GAListGenome < T > &, p2);
...
template < class T > int
	GAListGenome < T >::CycleCrossover(const GAGenome & p1,
					   const GAGenome & p2, GAGenome * c1, GAGenome * c2)
{
	const GAListGenome < T > &mom = DYN_CAST(const GAListGenome < T > &, p1);
	const GAListGenome < T > &dad = DYN_CAST(const GAListGenome < T > &, p2);
