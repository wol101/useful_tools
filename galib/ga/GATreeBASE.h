// $Header: /home/wis/Unix/cvsroot/galib/ga/GATreeBASE.h,v 1.1.1.1 2001/05/11 16:00:07 ws Exp $
/* ----------------------------------------------------------------------------
  treebase.h
  mbwall 25nov94
...
class GATreeBASE { public:
	...
	GANodeBASE *rt;				 // the root node of the tree
      ...
      private:
	GATreeBASE(const GATreeBASE &) {
	}					 // we don't allow copying
	GATreeBASE & operator = (const GATreeBASE &)
	{
		return *this;
	}					 // or op=
...
class GATreeIterBASE {
	...
	void operator() (const GATreeBASE & t) {
		...
		return (tree ? (node = tree->rt) : (GANodeBASE *) 0);
