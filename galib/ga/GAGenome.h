// $Header: /home/wis/Unix/cvsroot/galib/ga/GAGenome.h,v 1.1.1.1 2001/05/11 16:00:06 ws Exp $
/* ----------------------------------------------------------------------------
  genome.h
  mbwall 28jul94
...
// class.  So we have to use 0 as the defaults then check in the constructor.
...
#ifndef NO_STREAMS
	virtual int read(istream &) { GAErr(GA_LOC, className(), "read", gaErrOpUndef);
		return 0;
	} virtual int write(ostream &) const { GAErr(GA_LOC, className(), "write", gaErrOpUndef);
		return 0;
	}
#endif
	virtual int equal(const GAGenome &) const
	...
	Evaluator evaluator() const { return eval;
		...
		_evaluated = gaFalse;
		return (eval = f);
	} void initialize() { _evaluated = gaFalse;
		_neval = 0;
		(*init) (*this);
