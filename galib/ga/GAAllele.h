// $Header: /home/wis/Unix/cvsroot/galib/ga/GAAllele.h,v 1.1.1.1 2001/05/11 16:00:09 ws Exp $
/* ----------------------------------------------------------------------------
  allele.h
  mbwall 21mar95
...
template < class T > class GAAlleleSet
{
	...
	GAAlleleSet(const T & lower, const T & upper,
		    ...
		    GAAllele::INCLUSIVE):core(new GAAlleleSetCore < T > (lower, upper, lb, ub)) {
	}
	GAAlleleSet(const T & lower, const T & upper, const T & increment,
		    GAAllele::BoundType lb = GAAllele::INCLUSIVE,
		    GAAllele::BoundType ub =
		    GAAllele::INCLUSIVE):core(new GAAlleleSetCore < T > (lower,
									 upper,
									 increment, lb, ub)) {
	}
	...
	void unlink();
	int size() const { return core->sz;
	}
		// only meaninful for enumerated setshanks to RJG 15jul96------------------- */
	...
	T lower() const { return core->a[0];
	}
		// only for bounded setsetsrated setshanks to RJG 15jul96------------------- */
	...
	friend int operator == <T > (const GAAlleleSet < T > &,
				     const GAAlleleSet < T > &);
	friend int operator !=< T > (const GAAlleleSet < T > &, const GAAlleleSet < T > &);
#else
	friend int operator == (const GAAlleleSet < T > &,
				const GAAlleleSet < T > &);
...
#ifndef NO_STREAMS
template < class T > ostream & operator << (ostream & os, const GAAlleleSet < T > &arg)
{
	arg.write(os);
	return os;
}

template < class T > istream & operator >> (istream & is, GAAlleleSet < T > &arg)
{
	arg.read(is);
	return is;
}
