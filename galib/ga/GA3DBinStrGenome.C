// $Header: /home/wis/Unix/cvsroot/galib/ga/GA3DBinStrGenome.C,v 1.1.1.1 2001/05/11 16:00:10 ws Exp $
/* ----------------------------------------------------------------------------
  binstr3.C
  mbwall 19apr95
...
GA3DBinaryStringGenome::GA3DBinaryStringGenome(unsigned int width,
					       unsigned int height,
unsigned int depth, GAGenome::Evaluator f, void *u):
...
GA3DBinaryStringGenome::GA3DBinaryStringGenome(const GA3DBinaryStringGenome &
					       orig):GABinaryString(orig.GABinaryString::size()),
GAGenome()
{
	nx = minX = maxX = 0;
	ny = minY = maxY = 0;
...
#ifndef NO_STREAMS
int GA3DBinaryStringGenome::read(istream & is)
{
	static char c;
...
int GA3DBinaryStringGenome::write(ostream & os) const
{
...
void
	GA3DBinaryStringGenome::copy(const GA3DBinaryStringGenome & orig,
	...
	for (unsigned int k = 0; k < d; k++)
		for (unsigned int j = 0; j < h; j++)
			GABinaryString::copy(orig,
					     (t + k) * ny * nx + (s + j) * nx + r,
					     (z + k) * orig.ny * orig.nx + (y + j) * orig.nx +
					     x, w);
	_evaluated = gaFalse;
}
...
void
	GA3DBinaryStringGenome::set(unsigned int x, unsigned int y, unsigned int z,
				    unsigned int w, unsigned int h, unsigned int d)
{
	if (x + w > nx)
		w = nx - x;
...
void
	GA3DBinaryStringGenome::unset(unsigned int x, unsigned int y, unsigned int z,
				      unsigned int w, unsigned int h, unsigned int d)
{
	if (x + w > nx)
		w = nx - x;
...
void
	GA3DBinaryStringGenome::randomize(unsigned int x, unsigned int y,
...
void
	GA3DBinaryStringGenome::move(unsigned int x, unsigned int y, unsigned int z,
				     unsigned int srcx, unsigned int srcy,
				     unsigned int srcz, unsigned int w,
				     unsigned int h, unsigned int d)
{
	if (srcx + w > nx)
		w = nx - srcx;
...
int
	GA3DBinaryStringGenome::equal(const GA3DBinaryStringGenome & orig,
				      unsigned int x, unsigned int y, unsigned int z,
				      unsigned int srcx, unsigned int srcy,
				      unsigned int srcz, unsigned int w,
...
int
	GA3DBinaryStringGenome::UniformCrossover(const GAGenome & p1,
						 const GAGenome & p2, GAGenome * c1, GAGenome * c2)
{
	...
	} else if (c1 || c2)
	{
		GA3DBinaryStringGenome & sis = (c1 ?
						DYN_CAST(GA3DBinaryStringGenome &,
							 *c1) :
...
// It might break!
int
	GA3DBinaryStringGenome::EvenOddCrossover(const GAGenome & p1,
						 const GAGenome & p2, GAGenome * c1, GAGenome * c2)
{
	const GA3DBinaryStringGenome & mom = DYN_CAST(const GA3DBinaryStringGenome &, p1);
	const GA3DBinaryStringGenome & dad = DYN_CAST(const GA3DBinaryStringGenome &, p2);
...
int
	GA3DBinaryStringGenome::OnePointCrossover(const GAGenome & p1,
						  const GAGenome & p2, GAGenome * c1, GAGenome * c2)
{
	const GA3DBinaryStringGenome & mom = DYN_CAST(const GA3DBinaryStringGenome &, p1);
	const GA3DBinaryStringGenome & dad = DYN_CAST(const GA3DBinaryStringGenome &, p2);
