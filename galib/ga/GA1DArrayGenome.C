// $Header: /home/wis/Unix/cvsroot/galib/ga/GA1DArrayGenome.C,v 1.1.1.1 2001/05/11 16:00:04 ws Exp $
/* ----------------------------------------------------------------------------
  array1.C
  mbwall 25feb95
...
GA1DArrayIsHole(const GA1DArrayGenome < T > &, const GA1DArrayGenome < T > &, int, int, int);
...
GAArray < T > (length), GAGenome(DEFAULT_1DARRAY_INITIALIZER,
				 DEFAULT_1DARRAY_MUTATOR, DEFAULT_1DARRAY_COMPARATOR)
{
	evaluator(f);
	userData(u);
	nx = minX = maxX = length;
...
template < class T >
	GA1DArrayGenome < T >::GA1DArrayGenome(const GA1DArrayGenome < T > &orig):GAArray < T >
	(orig.sz), GAGenome()
{
...
template < class T > GAGenome * GA1DArrayGenome < T >::clone(GAGenome::CloneMethod flag) const
{
	...
	{					 // bizarre and strange __MWERKS bug (wis 21/9/00) - these enums need to be cast to ints to avoid link errors
		cpy->copy(*this);
	} else
	{
...
template < class T > int GA1DArrayGenome < T >::read(istream &)
{
...
template < class T > int GA1DArrayGenome < T >::write(ostream & os) const
{
...
template < class T > int
	GA1DArrayGenome < T >::resizeBehaviour(unsigned int lower, unsigned int upper)
{
	if (upper < lower)
	{
...
template < class T >
	GA1DArrayAlleleGenome < T >::GA1DArrayAlleleGenome(const GAAlleleSetArray <
							   T > &sa,
							   GAGenome::Evaluator f,
							   void *u):GA1DArrayGenome < T >
	(sa.size(), f, u)
{
	naset = sa.size();
...
template < class T >
	...
	T >::GA1DArrayAlleleGenome(const GA1DArrayAlleleGenome < T > &orig):GA1DArrayGenome < T >
	(orig.sz)
{
	naset = 0;
...
#ifndef NO_STREAMS
template < class T > int GA1DArrayAlleleGenome < T >::read(istream & is)
{
	return GA1DArrayGenome < T >::read(is);
}

template < class T > int GA1DArrayAlleleGenome < T >::write(ostream & os) const
{
...
template < class ARRAY_TYPE > float
	GA1DArrayGenome < ARRAY_TYPE >::ElementComparator(const GAGenome & a, const GAGenome & b)
{
...
template < class T > int
	GA1DArrayGenome < T >::UniformCrossover(const GAGenome & p1,
						const GAGenome & p2, GAGenome * c1, GAGenome * c2)
{
	...
	if (c1 && c2)
	{
		...
		if (sis.length() == bro.length() &&
		    mom.length() == dad.length() && sis.length() == mom.length())
		{
...
template < class T > int
	GA1DArrayGenome < T >::OnePointCrossover(const GAGenome & p1,
						 const GAGenome & p2, GAGenome * c1, GAGenome * c2)
{
	const GA1DArrayGenome < T > &mom = DYN_CAST(const GA1DArrayGenome < T > &, p1);
	const GA1DArrayGenome < T > &dad = DYN_CAST(const GA1DArrayGenome < T > &, p2);
	...
	if (c1 && c2)
	{
		    ...
		    bro.resizeBehaviour() == GAGenome::FIXED_SIZE)
		{
			if (mom.length() != dad.length() ||
			    sis.length() != bro.length() || sis.length() != mom.length())
			{
...
template < class T > int
	GA1DArrayGenome < T >::TwoPointCrossover(const GAGenome & p1,
						 const GAGenome & p2, GAGenome * c1, GAGenome * c2)
{
	const GA1DArrayGenome < T > &mom = DYN_CAST(const GA1DArrayGenome < T > &, p1);
	const GA1DArrayGenome < T > &dad = DYN_CAST(const GA1DArrayGenome < T > &, p2);
...
template < class T > int
	GA1DArrayGenome < T >::EvenOddCrossover(const GAGenome & p1,
						const GAGenome & p2, GAGenome * c1, GAGenome * c2)
{
	const GA1DArrayGenome < T > &mom = DYN_CAST(const GA1DArrayGenome < T > &, p1);
	const GA1DArrayGenome < T > &dad = DYN_CAST(const GA1DArrayGenome < T > &, p2);
...
template < class T > int
	GA1DArrayGenome < T >::PartialMatchCrossover(const GAGenome & p1,
						     const GAGenome & p2,
						     GAGenome * c1, GAGenome * c2)
{
	...
	} else if (c1 || c2)
	{
		...
		{
			for (j = 0;
			     j < sis.length() - 1 && sis.gene(j) != parent2->gene(index); j++);
			sis.swap(i, j);
		}
...
template < class T > int
GA1DArrayIsHole(const GA1DArrayGenome < T > &c,
...
template < class T > int
 GA1DArrayGenome < T >::OrderCrossover(const GAGenome & p1,
				       const GAGenome & p2, GAGenome * c1, GAGenome * c2)
{
	const GA1DArrayGenome < T > &mom = DYN_CAST(const GA1DArrayGenome < T > &, p1);
	const GA1DArrayGenome < T > &dad = DYN_CAST(const GA1DArrayGenome < T > &, p2);
...
template < class T > int
	GA1DArrayGenome < T >::CycleCrossover(const GAGenome & p1,
					      const GAGenome & p2, GAGenome * c1, GAGenome * c2)
{
	const GA1DArrayGenome < T > &mom = DYN_CAST(const GA1DArrayGenome < T > &, p1);
	const GA1DArrayGenome < T > &dad = DYN_CAST(const GA1DArrayGenome < T > &, p2);
