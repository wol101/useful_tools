...
f += '#$ -S /bin/bash\n'
f += '#$ -o OUTPUTFILE\n'
f += 'MPI_DIR=/opt/openmpi/\n'
f += 'ulimit -c 0\n'
f += 'EXEC="$HOME/Unix/cvs/UnifiedBootstrap2016/bin/UnifiedBootstrapAsync2016 --repeats %d --serverArgs -d 0 --clientArgs -d 0 -q "\n' % (nRepeats)
#f += '$MPI_DIR/bin/mpirun -np $NSLOTS -machinefile $TMP/machines $EXEC\n'
f += '$MPI_DIR/bin/mpirun -np $NSLOTS $EXEC\n'

# OUTPUTFILE is the name of the output file
jobSubmitFile = f

theTime = time.localtime(time.time())
outputFile = "rocks_%03dCore_%03dHour_%04d-%02d-%02d_%02d-%02d-%02d.txt" % (nCores, nHours, theTime[0], theTime[1], theTime[2], theTime[3], theTime[4], theTime[5])
            
jobSubmitFile = jobSubmitFile.replace("OUTPUTFILE", outputFile)

out = open("rocks_submit.sh", "w")
out.write(jobSubmitFile);
out.close()

qsub_command = "qsub -pe mpich %d rocks_submit.sh" % nCores
print qsub_command
