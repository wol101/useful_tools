...
#include "../GaitSym2019/src/DataFile.h"
#include "../GaitSym2019/src/MPIStuff.h"
#include "../GaitSym2019/src/XMLConverter.h"
#include "../GaitSym2019/src/GSUtil.h"
...
#define SYNCHRONOUS_MPI
        ...
        while (true)
        {
            ...
            double bestFitness = GSUtil::Double(ptrs[nTokens - 1]);
            delete [] ptrs;
...
void ApplyGenome(const char *inputXML, const char *inputGenome, const char *outputXML)
{
    ...
    double val;
    int ival, genomeSize;
    genomeData.ReadFile(std::string(inputGenome));
    ...
    std::string buf;
    myXMLConverter.GetFormattedXML(&buf);

    DataFile outputXMLData;
    outputXMLData.SetRawData(buf.data(), buf.size());
