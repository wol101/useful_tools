// wis - altered for double precision and removal of error messages
/*****************************************************************************
...
// wis - altered for double precision
#define DM_DOUBLE_PRECISION
#include <float.h>

//#define DM_HYDRODYNAMICS

// wis - altered for silent error reporting
// needs to be defined once in a source file
#ifdef DEFINING_DM_GLOBALS
int gDynamechsError = 0;
#else
extern int gDynamechsError;
#endif
...
// wis - dmu dmuFindObject definition
/** Find a dmObject with the given name
 */
class dmArticulation;
class dmObject;
DM_DLL_API dmObject *dmuFindObject(const char *name, dmArticulation *system);
...
// wis - altered for double precision
#ifdef DM_DOUBLE_PRECISION
   if (len > DBL_EPSILON * 10)
#else
   if (len > FLT_EPSILON * 10)
#endif
//   if (len > 0.000001)
   {
      quat[0] /= len;
...
// wis - altered to use the gDynamechsError global
      gDynamechsError = 1;        
