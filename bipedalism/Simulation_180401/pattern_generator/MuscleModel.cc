...
//============================================================================
// class MuscleModel : public LoggedForce
//============================================================================
...
MuscleModel::MuscleModel(Controller * controller,
       const CartesianVector attachmentLocal,
       const char * const otherBodyName, const CartesianVector attachmentOnOtherBody)
{
  m_reset_flag = true;
  m_dampingFactor = 50;
...
//----------------------------------------------------------------------------
void MuscleModel::setOtherBodyName(const char * const otherBodyName)
{
  if (m_otherBodyName)
    delete[]m_otherBodyName;
  m_otherBodyName = new char[strlen(otherBodyName) + 1];
...
//----------------------------------------------------------------------------
//    Summary: Compute the external force due to the muscle element
...
void MuscleModel::computeForce(dmABForKinStruct * val, SpatialVector f_contact)
{
  ...
  control = m_controller->Calculate(gSimulation->GetTime());
  if (control < 0)
  {
    control = 0; // muscles don't push
  }

  speedForce = m_dampingFactor * speed;

  // need to use a small integration stepsize for damping to work...
...
#else
  muscleForce =
    control * m_controlFactor;
#endif  
  if (muscleForce < 0)
    muscleForce = 0;     // muscles can't push

  // save the muscle force as the raw force value
  m_force = muscleForce;
  ...
  {
      cerr << "MuscleModel::computeForce\t";
      cerr << m_name << "\t";
      cerr << "muscleForce\t" << muscleForce << "\t";
    cerr << "speed\t" << speed << "\ttheDistance\t" << theDistance << "\t";
    cerr << "distanceVector\t" << distanceVector[XC] << "\t" <<
      distanceVector[YC] << "\t" << distanceVector[ZC] << "\t";
    cerr << "w_localOrigin\t" << w_localOrigin[XC] << "\t" <<
      w_localOrigin[YC] << "\t" << w_localOrigin[ZC] << "\t";
    cerr << "w_otherOrigin\t" << w_otherOrigin[XC] << "\t" <<
      w_otherOrigin[YC] << "\t" << w_otherOrigin[ZC] << "\t";
    cerr << "localVelocity\t" << localVelocity[XC] << "\t" <<
      localVelocity[YC] << "\t" << localVelocity[ZC] << "\t";
    cerr << "otherVelocity\t" << otherVelocity[XC] << "\t" <<
      otherVelocity[YC] << "\t" << otherVelocity[ZC] << "\t";
    cerr << "w_localVelocity\t" << w_localVelocity[XC] << "\t" <<
      w_localVelocity[YC] << "\t" << w_localVelocity[ZC] << "\t";
    cerr << "w_otherVelocity\t" << w_otherVelocity[XC] << "\t" <<
      w_otherVelocity[YC] << "\t" << w_otherVelocity[ZC] << "\t";
    cerr << "forceVector\t" << forceVector[XC] << "\t" <<
      forceVector[YC] << "\t" << forceVector[ZC] << "\n";
  }
