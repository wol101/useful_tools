...
void readModifiedContactModel(dmRigidBody * body, ifstream & cfg_ptr);
...
void readForces(dmRigidBody * body, ifstream & cfg_ptr)
{
	...
	while (strcmp(buffer, "End_Of_Force_Objects") != 0)
	{
		...
		if (strcmp(buffer, "ModifiedContactModel") == 0)
			readModifiedContactModel(body, cfg_ptr);
...
*/
	...
	if (strcmp(buffer, "StepController") == 0)
	{
		getStringParameter30(cfg_ptr, "Name", controllerName);
		getStringParameter30(cfg_ptr, "Controller_File", fileName);
		readConfigParameterLabel(cfg_ptr, "TimeStep");
		cfg_ptr >> stepTime;
		if (gSimulation->GetControllerDataFromFile())
			controller = new StepController(fileName);
		else
			controller = new StepController();
		controller->setName(controllerName);
		((StepController*)controller)->SetStepTime(stepTime);
		return controller;
	}
...
/*
	            Name			"part_name"
		    Number_of_Contact_Points	 nn
		    Contact_Locations		 xx   xx   xx
		                                 xx   xx   xx
*/						 

	getStringParameter30(cfg_ptr, "Name", contactName);
	...
	ModifiedContactModel *contact_model = new ModifiedContactModel();
	contact_model->setContactPoints(num_points, pos);
	contact_model->setName(contactName);
