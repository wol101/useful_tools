...
Torso::Torso():
  dmMobileBaseLink()
{
  // setState actually alters it's position argument so make a
  // local copy of kTorsoPosition
  double position[7];
  for (int i = 0; i < 7; i++) position[i] = kTorsoPosition[i];
  ...
  setState(position, (double *)kTorsoVelocity);
