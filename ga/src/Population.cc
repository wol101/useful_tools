  ...
  if (m_SelectionType == RouletteWheelSelection)
  {
    ...
    *parentRank = p;
    ...
    if (gDebug == Debug_Population)
      std::cerr << "Population::ChooseParent\tRouletteWheelSelection\tr\t" <<
...
void Population::Sort()
{
  ...
  if (m_SelectionType == RouletteWheelSelection)
  {
    ...
    for (i = 0; i < m_PopulationSize; i++)
    {
      ...
      if (gDebug == Debug_Population)
        std::cerr << "Population::Sort\tm_CumulativeFitness[\t" << i <<
    ...
    char *programName, bool readGenomeFlag)
{
  ...
  if (m_MaxRunning == 1)
  {
    ...
    if (extraArgs) objective.SetExtraArguments(extraArgs);
    if (readGenomeFlag) objective.SetReadGenomeFlag(true);
  ...
  else
  {
    ...
    if (readGenomeFlag)
    {
      for (i = 0; i < m_MaxRunning; i++)
      {
        runList[i].SetReadGenomeFlag(true);
      }
    }
    if (extraArgs)
    {
      for (i = 0; i < m_MaxRunning; i++)
      {
        runList[i].SetExtraArguments(extraArgs);
      }
    }
    // loop through the population
    for (i = from; i < to; i++)
    ...
    int restartOnError)
{
  ...
  for (i = 0; i < runListSize; i++)
  {
    ...
    if (m_Population[from + i]->GetFitnessValid() == false)
    {
      ...
      // if there's space start up a new process
      do
      {
        preferredHost = Objective::GetPreferredHost(runList[i].GetProgramName());
        if (preferredHost) runList[i].Run(preferredHost);
        else usleep(10000); // sleep for 10ms - might need to tweak

        // check to see if any of our current processes have finished
        // because this makes sure the child fitness message is read
        for (j = 0; j <= i; j++) runList[j].CheckRunning();
      } while (preferredHost == 0);
    }
  }
  ...
  do
  {
    ...
    if (stillWaiting == true) usleep(10000);
    
    // double check by counting tasks
    if (Objective::CountTasks(programName) == 0) break;
  ...
  for (i = 0; i < runListSize; i++)
  {
    ...
    //  std::cerr << "m_Population[" << runList[i].GetID() << "]->GetFitnessValid() is true\n";
...
std::ostream& operator<<(std::ostream &out, Population &g)
{
  int i;
...
std::istream& operator>>(std::istream &in, Population &g)
{
  int i;
  int populationSize;
