...
int main(int argc, char *argv[])
{
  ...
  status = params.RetrieveParameter("hillClimbEvery", &hillClimbEvery);
  ...
  outFile << "hillClimbEvery\t" << hillClimbEvery << "\n";
...
/* debugging code */
  ...
  for (j = startingGenerationNumber + 1; j < maxGenerations; j++)
  {
    ...
    if (hillClimbEvery > 0)
    {
      if (j % hillClimbEvery == hillClimbEvery - 1)
      {
        thePopulation.CalculateFitness(0, populationSize, 0, "hillclimb", true);
      }
    }
#endif
