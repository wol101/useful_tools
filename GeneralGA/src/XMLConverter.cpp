...
XMLConverterGA::XMLConverterGA(XMLConverterGA &converter)
{
    ...
    {
        parser = new ExpressionParserGA(*converter.m_SmartSubstitutionParserComponents[i]);
...
int XMLConverterGA::LoadBaseXMLFile(const char *filename)
{
...
xmlChar *XMLConverterGA::GetFormattedXML(int *docTxtLen)
{
    if (m_ConversionType == SmartSubstitution)
    {
...
int XMLConverterGA::ApplyGenome(Genome *g)
{
    if (m_ConversionType == SmartSubstitution)
    {
...
void XMLConverterGA::NewCyclicDriver(xmlNodePtr parent, const char *target,
...
void XMLConverterGA::NewStepDriver(xmlNodePtr parent, const char *target,
...
void XMLConverterGA::FlexorEncode(double *in, int n, double *out)
{
    int i;
    for (i = 0; i < n; i++) out[i] = in[i] > 0 ? in[i]: 0;
}

void XMLConverterGA::ExtensorEncode(double *in, int n, double *out)
{
    int i;
    for (i = 0; i < n; i++) out[i] = in[i] < 0 ? -in[i]: 0;
}
...
void XMLConverterGA::DoSmartSubstitution(char *dataPtr)
{
    char *ptr1 = dataPtr;
    string *s;
