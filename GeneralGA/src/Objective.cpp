...
bool Objective::Run()
{
  ...
  try
  {
    ...
    if (m_Client->waitfor(100) == false) 
    {
        if (gDebug == Debug_Sockets) gSocketLogFile << "Error reading control word\n";
        throw(0); // nothing to read
    }
    m_Client->read(buffer, kSocketMaxMessageLength);
    if (strcmp(buffer, kSocketRequestTaskMessage) != 0) 
    {
        if (gDebug == Debug_Sockets) gSocketLogFile << "Error matching control word\n";
        throw (0);
    }
...
bool Objective::CheckRunning()
{
  ...
  catch (int e)
  {
    ...
    m_Client = 0;
    m_RunningFlag = false;
    while (Run() == false) usleep(gUsleep);
