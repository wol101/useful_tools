...
void GaussianMutate(Genome *genome, double mutationChance)
{
    ...
    if (genome->GetGaussianSD(location) > 0)
    {
        ...
        if (gCircularMutation == 0)
        {
            if (v < genome->GetLowBound(location)) v = genome->GetLowBound(location);
            else if (v > genome->GetHighBound(location)) v = genome->GetHighBound(location);
        }
        else
        {
            if (v < genome->GetLowBound(location)) v = genome->GetHighBound(location) - (genome->GetLowBound(location) - v);
            else if (v > genome->GetHighBound(location)) v = genome->GetLowBound(location) + (v - genome->GetHighBound(location));
            // rare occasions this may go crazy because gaussian mutations can be very large
            if (v < genome->GetLowBound(location)) v = genome->GetLowBound(location);
            else if (v > genome->GetHighBound(location)) v = genome->GetHighBound(location);
        }
        ...
        if (CoinFlip(mutationChance)) 
        {
            ...
            if (genome->GetGaussianSD(i) > 0)
            {
                ...
                if (gCircularMutation == 0)
                {
                    ...
                    else if (v > genome->GetHighBound(i)) v = genome->GetHighBound(i);
                }
                else
                {
                    if (v < genome->GetLowBound(i)) v = genome->GetHighBound(i) - (genome->GetLowBound(i) - v);
                    else if (v > genome->GetHighBound(i)) v = genome->GetLowBound(i) + (v - genome->GetHighBound(i));
                    // rare occasions this may go crazy because gaussian mutations can be very large
                    if (v < genome->GetLowBound(i)) v = genome->GetLowBound(i);
                    else if (v > genome->GetHighBound(i)) v = genome->GetHighBound(i);
                }
