...
ExpressionParserGA::ExpressionParserGA()
{
}

ExpressionParserGA::ExpressionParserGA(ExpressionParserGA &parser)
{
    ExpressionParserTokenGA *parserToken;
    unsigned int i;
    for (i = 0; i < parser.m_ExpressionParserTokenList.size(); i++)
    {
        parserToken = new ExpressionParserTokenGA(*parser.m_ExpressionParserTokenList[i]);
        m_ExpressionParserTokenList.push_back(parserToken);
    }
}
...
// initialise the ExpressionParserGA based on a string
// returns 0 if successful
// the character input[length] must exist (can be '\0')
int ExpressionParserGA::CreateFromString(char *input, int length)
{
    int index = 0;
    int parenthesisStart;
    int parenthesisCount;
    int functionStart;
    ExpressionParserTokenGA *expressionParserToken;
    bool finished;
    char byte;
    char *ptr;
    int i, j;
    bool expectingNumber = true;
    ExpressionParserGA *expressionParser;
    int nargs;
...
double ExpressionParserGA::Evaluate(Genome *genome)
{
    return EvaluateParser(this, genome);
}

// Evaluate expression from genome
double ExpressionParserGA::EvaluateParser(ExpressionParserGA *parser, Genome *genome)
{
    double lhs, rhs;
    unsigned int i;
...
// Evaluate a token
double ExpressionParserGA::EvaluateToken(ExpressionParserTokenGA *token, Genome *genome)
{
    switch(token->type)
    {
        case EPNumber:
...
double ExpressionParserGA::EvaluateVariable(ExpressionParserTokenGA *token, Genome *genome)
{
    if (token->name != "g")
    {
...
// Evaluate a function
double ExpressionParserGA::EvaluateFunction(ExpressionParserTokenGA *token, Genome *genome)
{
    switch (token->index)
    {
        case 0: // if function
...
double ExpressionParserGA::EvaluateOperator(double lhs, double rhs, int index)
{
    switch(index)
    {
        case '+':
