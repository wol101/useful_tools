...
//============================================================================
// class FBMuscleModel : public dmForce
//============================================================================

//----------------------------------------------------------------------------
FBMuscleModel::FBMuscleModel()
{
	m_reset_flag = true;
	m_dampingFactor = 50;
	m_controller = 0;
...
FBMuscleModel::FBMuscleModel(Controller * controller,
...
//----------------------------------------------------------------------------
void FBMuscleModel::setOtherBodyName(char *otherBodyName)
{
	if (m_otherBodyName)
		delete[]m_otherBodyName;
	m_otherBodyName = new char[strlen(otherBodyName) + 1];
...
void FBMuscleModel::computeForce(dmABForKinStruct * val, SpatialVector f_contact)
{
	int j;
	...
	// assert(control >= 0);
	if (control < 0) control = 0; // fix to greater than zero
	...
	if (muscleForce > m_maxForce)
		muscleForce = m_maxForce;	// ans there is a limit
	...
	m_LengthSensor.SetValue(theDistance);
	m_ForceSensor.SetValue(muscleForce);
}
