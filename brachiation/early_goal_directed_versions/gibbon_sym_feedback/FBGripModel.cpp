// FBGripModel - this class models a simple grip on the substrate.
...
//============================================================================
// class FBGripModel : public dmForce
//============================================================================

//----------------------------------------------------------------------------
FBGripModel::FBGripModel()
{
	m_reset_flag = true;
...
FBGripModel::FBGripModel(Bar * bar, Controller * controller)
{
	m_reset_flag = true;
	m_num_contact_points = 0;
	m_contact_flag = 0;
...
//----------------------------------------------------------------------------
void FBGripModel::setContactPoints(unsigned int num_contact_points, CartesianVector * contact_pos)
{
	m_reset_flag = true;
...
//----------------------------------------------------------------------------
bool FBGripModel::getContactPoint(unsigned int index, CartesianVector pos) const
{
...
void FBGripModel::computeForce(dmABForKinStruct * val, SpatialVector f_contact)
{
	...
	for (unsigned int i = 0; i < m_num_contact_points; i++)
	{
		    ...
		    (m_contact_flag[i] == true && theDistance < m_reachDistance + m_bar->GetRadius()))	// contact
		{
			...
			// assert(grip >= 0);
			if (grip < 0) grip = 0; // fix to greater than zero
	...
	// send out the sensor values
	m_XSensor.SetValue(f_contact[3]);
	m_YSensor.SetValue(f_contact[4]);
	m_ZSensor.SetValue(f_contact[5]);
}

// draw thyself!
void FBGripModel::draw()
{
