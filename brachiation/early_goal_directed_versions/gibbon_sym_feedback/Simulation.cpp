...
// Link the sensors up to any integrators
  for (int i = 0; i < gControllerList->GetSize(); i++) 
  {
  	if (strcmp(gControllerList->GetEntry(i)->GetType(), "IntegratingController") == 0)
  	{
			for (int j = 0; j < gSensorList->GetSize(); j++) 
			  ((IntegratingController *)gControllerList->GetEntry(i))->AddSensor(gSensorList->GetEntry(j));
		}
	}
}
...
void Simulation::UpdateSimulation()
{
	...
	// bump the sensor data
	for (int i = 0; i < gSensorList->GetSize(); i++) gSensorList->GetEntry(i)->Bump();
