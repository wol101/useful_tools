...
void readGripModel(dmRigidBody * body, ifstream & cfg_ptr);
...
void readForces(dmRigidBody * body, ifstream & cfg_ptr)
{
	...
	while (strcmp(buffer, "End_Of_Force_Objects") != 0)
	{
		...
		if (strcmp(buffer, "GripModel") == 0)
			readGripModel(body, cfg_ptr);
...
void readGripModel(dmRigidBody * body, ifstream & cfg_ptr)
{
	...
	char gripName[NAME_SIZE];
	double reachDistance;
	double dampingFactor;
	double controlFactor;
	double maxForce;
	Controller *controller;
	FBGripModel *gripModel;
/*
	Name					"PartName"
    	Number_of_Contact_Points		xx
	Contact_Locations			xx xx xx
						xx xx xx
	Reach_Distance				xx
	Damping_Factor				xx
	Control_Factor              		xx
...
*/
	getStringParameter30(cfg_ptr, "Name", gripName);
	...
	{
		cfg_ptr >> pos[i][0] >> pos[i][1] >> pos[i][2];
	}

	readConfigParameterLabel(cfg_ptr, "Reach_Distance");
	cfg_ptr >> reachDistance;
	...
	readConfigParameterLabel(cfg_ptr, "Max_Force");
	cfg_ptr >> maxForce;

	controller = readController(cfg_ptr);

	gripModel = new FBGripModel(gSimulation->GetBar(), controller);
	gripModel->setContactPoints(num_points, pos);
	gripModel->setReachDistance(reachDistance);
	gripModel->setDampingFactor(dampingFactor);
	gripModel->setControlFactor(controlFactor);
	gripModel->setMaxForce(maxForce);
	gripModel->setName(gripName);

	body->addForce(gripModel);
...
*/
	...
	if (strcmp(buffer, "IntegratingController") == 0)
	{
		getStringParameter30(cfg_ptr, "Name", controllerName);
		getStringParameter30(cfg_ptr, "Controller_File", fileName);
		if (gSimulation->GetControllerDataFromFile())
			controller = new IntegratingController(fileName);
		else
			controller = new IntegratingController();
		controller->setName(controllerName);
		return controller;
	}
...
*/
	...
	muscleModel = new FBMuscleModel(controller, localAttach, otherBodyName, otherAttach);
	...
	muscleModel->setMaxForce(maxForce);
