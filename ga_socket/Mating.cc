...
void GaussianMutate(Genome *genome, double mutationChance, double gaussianSD)
{
  ...
  if (gaussianSD >= 0)
    v = genome->GetGene(location) + RandomUnitGaussian() * gaussianSD;
  else
  {
    w = genome->GetGene(location);
    if ( w < (-gaussianSD) / 10 )
      v = w + RandomUnitGaussian() * gaussianSD / 10;
    else
      v = w + RandomUnitGaussian() * gaussianSD * w;
  }
  if (v < low) v = low;
  if (v > high) v = high;
...
void MultipleGaussianMutate(Genome *genome, double mutationChance, double gaussianSD)
{
  ...
  for (i = 0; i < genomeLength; i++)
  {
    ...
    {
      if (gaussianSD >= 0)
        v = genome->GetGene(i) + RandomUnitGaussian() * gaussianSD;
      else
      {
        w = genome->GetGene(i);
        if ( w < (-gaussianSD) / 10 )
