...
if (GetValueFromTokenList("eulerToQuaternionCommand", $eulerToQuaternionCommand, @tokenList)) { die "eulerToQuaternionCommand undefined"; }
if (GetValueFromTokenList("quaternionToEulerCommand", $quaternionToEulerCommand, @tokenList)) { die "quaternionToEulerCommand undefined"; }
if (GetValueFromTokenList("correctQuaternionFlag", $correctQuaternionFlag, @tokenList)) { die "correctQuaternionFlag undefined"; }
  ...
  $kRangeControlPercent += $kRangeControlIncrementPercent)
{
  ...
  for ($i = 0; $i <= $#tokenList; $i++)
  {
     ...
     {
        if ($correctQuaternionFlag)
        {
     	   @newQuaternion = CorrectQuaternion($tokenList[$i + 1], $tokenList[$i + 2], $tokenList[$i + 3], $tokenList[$i + 4]);
     ...
     if ($tokenList[$i] eq "kTorsoVelocity")
     {
        printf(OUT "\nkTorsoVelocity %.16g %.16g %.16g\n%.16g %.16g %.16g\n",
           0,   
           $tokenList[$i + 2],   
           0,   
           $tokenList[$i + 4],   
...
sub CorrectQuaternion
{
   my $command;
   my $theTokenList = ();

   # covert to Euler
   $command = $quaternionToEulerCommand . " " . $_[0] . " " . $_[1] . " " . $_[2] . " " . $_[3] . " > temp.txt";
   print "$command\n";
   system $command;
   ...
   $command = $eulerToQuaternionCommand . " -90 " . $theTokenList[1] . " 180 > temp.txt";
   print "$command\n";
   system $command;
   @theTokenList = ConvertFileToTokens("temp.txt");
