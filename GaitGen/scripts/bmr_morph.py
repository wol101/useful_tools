...
def main():
	...
	startBMR = float(GetValueFromTokenList(tokenList, "startBMR"))
	endBMR = float(GetValueFromTokenList(tokenList, "endBMR"))
	minEnergyLimit = float(GetValueFromTokenList(tokenList, "minEnergyLimit"))
    ...
    while (kRangeControlPercent * kRangeControlIncrementPercent) <= (kRangeControlEndPercent * kRangeControlIncrementPercent):
        kRangeControl = kRangeControlPercent / 100.0 
	
	# calculate run parameters
	currentBMR = startBMR + kRangeControl * (endBMR - startBMR)
	energyLimit = 10.0 * currentBMR + minEnergyLimit;
        if energyLimit < 100: energyLimit = 100;

        # build the working config file

        theOutput = open(workingConfigFile, 'w')
	theOutput.write("kBMR %f\n\n" % (currentBMR))
	...
	theOutput.write("extraArguments	\"--energyLimit %f --timeLimit 100 --numStartupPhases 0\"\n\n" % energyLimit)
        ...
        command = "%s -c %s -g %s --energyLimit %f" % (objectiveCommand, workingConfigFile, workingGenome, energyLimit)
        print command
        os.system(command)  
