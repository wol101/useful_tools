...
float gTimeLimit = 100;
float gEnergyLimit = 100000;
...
static int numStartupPhases = 0;
static int numCyclicPhases = 6; // skipping 10; walking 6
static int numAllelesPerPhase = 7;
static int outputStateAtPhase = -1;
static bool summaryFlag = false;
static FitnessType gFitnessType = DistanceTravelled;
static bool correctTorsoOutput = false;
...
void ParseArguments(int argc, char ** argv)
{
    ...
    for (i = 0; i < argc; i++)
    {
            ...
            strcmp(argv[i], "-k") == 0)
        {
            ...
            if (strcmp(argv[i], "--correctTorsoOutput")  == 0)
            {
                correctTorsoOutput = true;
            }
        else
            ...
            if (strcmp(argv[i], "--distanceTravelled") == 0)
            {
                gFitnessType = DistanceTravelled;
            }
        else
            if (strcmp(argv[i], "--kinematicMatch") == 0)
            {
                gFitnessType = KinematicMatch;
            }
        else
                ...
                strcmp(argv[i], "-?") == 0)
            {
                ...
                cerr << "number of .obj files or graphical display).\n\n";
                ...
                cerr << "--correctTorsoOutput\n";
                cerr << "Rotates the torso orientation output so it is on the y = 0 plane\n";
                cerr << "and zeros out of plane positions and velocities\n\n";
                cerr << "-e n, --debug n\n";
                ...
                cerr << "--displaySkip n\n";
                cerr << "Displays a new frame every n calculations (default 200)\n\n";
                ...
                cerr << "Sets the number of startup phases (default 0)\n\n";
                ...
                cerr << "Uses distance travelled as the fitness criteria\n\n";
                cerr << "--kinematicMatch\n";
                cerr << "Uses target kinematics as the fitness criteria\n\n";
...
int ReadModel()
{
    ...
    gSimulation->SetFitnessType(gFitnessType);
...
std::istringstream in(buf);
    ...
    {
        cerr << "Error reading " << genomeFilenamePtr << "\n";
        exit(1);
    }
    inFile >> genome;
    inFile.close();
    ...
    for (i = 0; i < genomeLength; i++) genomeData[i] = genome.GetGene(i);
...
pgd::Vector v = pgd::QVRotate(rotation, pgd::Vector(1, 0, 0));
        v.y = 0;
        v.Normalize();
        double yAxisRotation = acos(v * pgd::Vector(1, 0, 0)); // always gives a positive angle
        if (v.z > 0) yAxisRotation = -yAxisRotation; // this is for rotations about the y axis
pgd::Quaternion xAxisRotationQ = pgd::MakeQFromAxis(1, 0, 0, M_PI/2);
pgd::Quaternion yAxisRotationQ = pgd::MakeQFromAxis(0, 1, 0, yAxisRotation);
        // get combined quaternion for rotation about x first, then y
pgd::Quaternion correctedRotation = yAxisRotationQ * xAxisRotationQ;
        out << "kTorsoPosition "
            << correctedRotation.v.x << " "  << correctedRotation.v.y << " "
            << correctedRotation.v.z << " "  << correctedRotation.n << " "
            << 0 << " 0 " << q[6] << "\n";
        out << "kTorsoVelocity 0 " << qd[1] << " 0 " << qd[3] << " 0 " << qd[5] << "\n\n";
    }
