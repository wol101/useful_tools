...
// this version changes the data pointed to
...
// this is simply a const char version
int XMLConverter::LoadBaseXMLString(const char *dataPtr)
{
    char *newBuf = new char[strlen(dataPtr)];
    strcpy(newBuf, dataPtr);
    int result = LoadBaseXMLString(newBuf);
    delete [] newBuf;
    return result;
}
