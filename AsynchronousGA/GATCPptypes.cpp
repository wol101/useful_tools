...
#endif
{
    ...
    if (startPopulation->GetPopulationSize() != gPrefs.populationSize)
    {
        ...
        if (m_logLevel > 0)
        {
            message = QString("Info: Starting population size %1 does not match specified population size %2 - using standard fixup").arg(startPopulation->GetPopulationSize()).arg(gPrefs.populationSize);
            QMetaObject::invokeMethod(m_callingWidget, "appendProgress", Qt::QueuedConnection, Q_ARG(QString, message));
        }
#endif
...
int GAMain::Evolve(Population *startPopulation, Population *thePopulation, std::ostream *outFile, const char *dirname)
{
    ...
    try
    {
        ...
        socketNum = svr.bindall(gPrefs.tcpPort);
    }
    catch(pt::estream* e)
    {
#ifdef USE_QT_AGA
    ...
    while (returnCount < gPrefs.maxReproductions && stopSendingFlag == false)
    {
            ...
            if (st) // anything to receive?
            {
                ...
                if (numBytes == TCPIPMessage::StandardMessageSize && strcmp(commsBuffer.get(), "result") == 0 && memcmp(tcpMessage->md5, m_md5, sizeof(m_md5)) == 0)
                {
                    ...
                    if (iter != runningList.end())
                    {
                        ...
                        if (returnCount % gPrefs.saveBestEvery == gPrefs.saveBestEvery - 1 || returnCount == 1)
                        {
                        ...
                        if (returnCount % gPrefs.savePopEvery == gPrefs.savePopEvery - 1 || returnCount == 1)
                        {
                        ...
                        if (returnCount % gPrefs.improvementReproductions == gPrefs.improvementReproductions - 1)
                        {
                            ...
                            if (m_logLevel > 1)
                            {
                                message = QString("Fitness change for %1 reproductions is %2").arg(gPrefs.improvementReproductions).arg(maxFitness - lastMaxFitness);
                                QMetaObject::invokeMethod(m_callingWidget, "appendProgress", Qt::QueuedConnection, Q_ARG(QString, message));
                            }
#endif
...
void GAMain::SetTCPPort(int port)
{
    gPrefs.tcpPort = port;
}
