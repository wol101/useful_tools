...
//  TIFFWrite.cpp
//
//  Created by Bill Sellers on Wed Sep 08 2004.
...
// note use "cpp -dM" to find all machine defined cpp macros
...
#ifdef USING_64_BIT_POINTERS
    fileHeader.theHeader.IFDOffset = (uint64_t)(&(fileHeader.nEntries)) - (uint64_t)(&fileHeader);
#else
    fileHeader.theHeader.IFDOffset = (uint32_t)(&(fileHeader.nEntries)) - (uint32_t)(&fileHeader);
#endif
...
#ifdef USING_64_BIT_POINTERS
        fileHeader.entries[2].valueOrOffset = (uint64_t)(&(fileHeader.pixelsPerSample[0])) - (uint64_t)(&fileHeader);
#else
        fileHeader.entries[2].valueOrOffset = (uint32_t)(&(fileHeader.pixelsPerSample[0])) - (uint32_t)(&fileHeader);
#endif
...
#ifdef USING_64_BIT_POINTERS
    fileHeader.entries[9].valueOrOffset = (uint64_t)(&(fileHeader.xres[0])) - (uint64_t)(&fileHeader);
#else
    fileHeader.entries[9].valueOrOffset = (uint32_t)(&(fileHeader.xres[0])) - (uint32_t)(&fileHeader);
#endif
...
#ifdef USING_64_BIT_POINTERS
    fileHeader.entries[10].valueOrOffset = (uint64_t)(&(fileHeader.yres[0])) - (uint64_t)(&fileHeader);
#else
    fileHeader.entries[10].valueOrOffset = (uint32_t)(&(fileHeader.yres[0])) - (uint32_t)(&fileHeader);
#endif
