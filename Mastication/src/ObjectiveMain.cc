...
static int numStartupPhases = 256;
static int numCyclicPhases = 0;
static int numAllelesPerPhase = 9;
  ...
  while(gRunTimeLimit == 0 || runTime <= gRunTimeLimit)
  {
    ...
    else
    {
      ...
      < gEnergyLimit)
      {
        ...
        {
            if (gDebug == MainDebug) cerr << "main: Break due to catastrrophy\n";
            break;
        }
      }

      if (gDebug == MainDebug)
          cerr << "main:\tgSimulation->GetTime()\t" << gSimulation->GetTime() <<
              "\tgTimeLimit\t" << gTimeLimit << "\tgSimulation->GetMetabolicEnergy()\t" <<
...
void ParseArguments(int argc, char ** argv)
{
  ...
  for (i = 0; i < argc; i++)
  {
      ...
      strcmp(argv[i], "-?") == 0)
    {
      ...
      cerr << "Sets the number of startup phases (default 256)\n\n";
      cerr << "--numCyclicPhases n\n";
      cerr << "Sets the number of cyclic phases (default 0)\n\n";
      cerr << "--numAllelesPerPhase n\n";
      cerr << "Sets the number of alleles per phase (default 9)\n\n";
...
OutputProgramState(ostream &out)
{
   ...
   Mandible *mandible = gSimulation->GetMandible();
   mandible->getState(q, qd);
   
   out << "kMandiblePosition";
   for (i = 0; i < 7; i++) out << " " << q[i];
   out << "\n";
   out << "kMandibleVelocity";
