...
int main(int argc, char *argv[])
{
   ...
   for (i = 1; i < argc; i++)
   {
      ...
      if (strcmp(argv[i], "--parallelObjectives") == 0)
      {
         i++;
         if (i >= argc) return 1;
         sscanf(argv[i], "%d", &parallelObjectives);
         if (parallelObjectives < 1) parallelObjectives = 1;
      }
      else
   ...
   bestPopulation.SetMaxRunning(parallelObjectives);
   bestPopulation.CalculateFitness(0, populationSize, extraArguments, 0, false);
   bestPopulation.Sort();
   ...
   newPopulation.SetMaxRunning(parallelObjectives);
   ...
   for (i = 1; i < maxTrials; i++)
   {
      ...
      newPopulation.CalculateFitness(0, populationSize, extraArguments, 0, false);
      newPopulation.Sort();
