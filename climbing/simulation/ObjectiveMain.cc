...
Reads filename rather than the default ModelData.txt as the config data\n\n\
...
esc - quits\n\n";
  ...
  // Set the ladder
  gSimulation->GetLadder()->SetDimensions(5, 4, 0,
      2, 10, 0.4, 0.03);
  if (displayFlag) gSimulation->GetLadder()->InitialiseGraphics();
...
// the inevitable fitness function
...
CalculateFitness()
{
  ...
  unsigned theTorsoIndex = gSimulation->GetRobot()->getLinkIndex(myLink);
  const dmABForKinStruct *theTorsoDmABForKinStruct = gSimulation->GetRobot()->getForKinStruct(theTorsoIndex);
  // get the indices of RightArm
  myLink = (dmLink *) dmuFindObject(kRightArmPartName, gSimulation->GetRobot());
  unsigned theRightArmIndex = gSimulation->GetRobot()->getLinkIndex(myLink);
  const dmABForKinStruct *theRightArmDmABForKinStruct = gSimulation->GetRobot()->getForKinStruct(theRightArmIndex);
  // get the indices of LeftArm
  myLink = (dmLink *) dmuFindObject(kLeftArmPartName, gSimulation->GetRobot());
  unsigned theLeftArmIndex = gSimulation->GetRobot()->getLinkIndex(myLink);
  const dmABForKinStruct *theLeftArmDmABForKinStruct = gSimulation->GetRobot()->getForKinStruct(theLeftArmIndex);
  // get the indices of RightThigh
  myLink = (dmLink *) dmuFindObject(kRightThighPartName, gSimulation->GetRobot());
  unsigned theRightThighIndex = gSimulation->GetRobot()->getLinkIndex(myLink);
  const dmABForKinStruct *theRightThighDmABForKinStruct = gSimulation->GetRobot()->getForKinStruct(theRightThighIndex);
  // get the indices of LeftThigh
  myLink = (dmLink *) dmuFindObject(kLeftThighPartName, gSimulation->GetRobot());
  unsigned theLeftThighIndex = gSimulation->GetRobot()->getLinkIndex(myLink);
  const dmABForKinStruct *theLeftThighDmABForKinStruct = gSimulation->GetRobot()->getForKinStruct(theLeftThighIndex);
  ...
  while (gSimulation->GetTime() < gTimeLimit)
  {
    ...
    if (theTorsoDmABForKinStruct->p_ICS[2] < -1.0) 
    {
      successFlag = false;
      break;
    }
    ...
    energy += CalculateEnergyCost(gSimulation->GetCPG()->GetLeftShoulderAbductor());
    energy += CalculateEnergyCost(gSimulation->GetCPG()->GetLeftShoulderExtensor());
    energy += CalculateEnergyCost(gSimulation->GetCPG()->GetLeftShoulderFlexor());
    energy += CalculateEnergyCost(gSimulation->GetCPG()->GetRightShoulderAbductor());
    energy += CalculateEnergyCost(gSimulation->GetCPG()->GetRightShoulderExtensor());
    energy += CalculateEnergyCost(gSimulation->GetCPG()->GetRightShoulderFlexor());
    energy += CalculateEnergyCost(gSimulation->GetCPG()->GetLeftElbowExtensor());
    energy += CalculateEnergyCost(gSimulation->GetCPG()->GetLeftElbowFlexor());
    energy += CalculateEnergyCost(gSimulation->GetCPG()->GetRightElbowExtensor());
    energy += CalculateEnergyCost(gSimulation->GetCPG()->GetRightElbowFlexor());
    energy += CalculateEnergyCost(gSimulation->GetCPG()->GetLeftWristExtensor());
    energy += CalculateEnergyCost(gSimulation->GetCPG()->GetLeftWristFlexor());
    energy += CalculateEnergyCost(gSimulation->GetCPG()->GetRightWristExtensor());
    energy += CalculateEnergyCost(gSimulation->GetCPG()->GetRightWristFlexor());
    
    deltaP += Util::Magnitude3x1(theLeftArmDmABForKinStruct->v) * gSimulation->GetTimeIncrement();
    deltaP += Util::Magnitude3x1(theRightArmDmABForKinStruct->v) * gSimulation->GetTimeIncrement();
    deltaP += Util::Magnitude3x1(theLeftThighDmABForKinStruct->v) * gSimulation->GetTimeIncrement();
    deltaP += Util::Magnitude3x1(theRightThighDmABForKinStruct->v) * gSimulation->GetTimeIncrement();
  ...
  if (successFlag) score = 10000 - energy - 1000 * deltaP;
  else score = 0;
