...
//============================================================================
// class LadderContactModel : public dmForce
//============================================================================

//----------------------------------------------------------------------------
LadderContactModel::LadderContactModel():
  ExtendedForce(),
  m_reset_flag(true),
...
//----------------------------------------------------------------------------
void
LadderContactModel::setContactPoints(int num_contact_points,
...
//----------------------------------------------------------------------------
bool LadderContactModel::getContactPoint(int index, CartesianVector pos) const
{
...
//                   body's CS
//----------------------------------------------------------------------------
void LadderContactModel::computeForce(const dmABForKinStruct & val, SpatialVector f_contact)
{
  int i, j;
  double penetration;
  double ptemp, vtemp;
  ...
  CartesianVector normal, current_pos, surface_pos;
  ...
  for (i = 0; i < m_num_contact_points; i++)
  {
    ...
    penetration =
      gSimulation->GetLadder()->GetDepth(current_pos, surface_pos, normal);

    if (penetration == 0)  // No contact
    {
      // Reset flags.
      m_contact_flag[i] = false;
      m_sliding_flag[i] = false;
    ...
    else         // Contact!
    {
      if (!m_contact_flag[i] || m_reset_flag)  // set initial contact Pos.
      {
        ...
        cerr << "Contact " << flush;
#endif
        m_initial_contact_pos[i][0] = surface_pos[0];
        m_initial_contact_pos[i][1] = surface_pos[1];
        m_initial_contact_pos[i][2] = surface_pos[2];
      }
...
/*        if (m_sliding_flag[i])
  ...
  if (gDebug == ContactDebug)
  {
    ...
    cerr << "LadderContactModel::computeForce" << "\t";
    cerr << m_name << "\tWorld\t";
    for (j = 0; j < 6; j++)
      cerr << world[j] << "\t";
    cerr << "Local\t";
...
bool LadderContactModel::AnyContact(void)
{
  for (int i = 0; i < m_num_contact_points; i++)
  {
...
void LadderContactModel::GetContactForce(SpatialVector f)
{
  for (int i = 0; i < 6; i++)
  {
...
void LadderContactModel::draw() const
{
  if (m_drawValid)
  {
    for (int i = 0; i < m_num_contact_points; i++)
