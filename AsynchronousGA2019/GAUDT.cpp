...
int main(int argc, char *argv[])
{
    ...
    if (udtUpDown.status())
    {
        std::cerr << "Error: failed to initialise UDT\n");
        return 1;
    }
...
int GAMain::Evolve(Population *startPopulation, Population *thePopulation, std::ostream *outFile, const std::string &dirname)
{
    ...
    UDTSOCKET server = UDTWrapper::StartServer(std::to_string(g_prefs->tcpPort));
    if (server == UDT::INVALID_SOCK)
    {
        ReportProgress(ToString("Evolve: Error setting up server - port %d probably in use", g_prefs->tcpPort), 0);
        return __LINE__;
    }
    ReportProgress(ToString("UDT/IP port %d open", g_prefs->tcpPort), 1);
    UDTCloseSocketGuard stopServer(server);
    ...
    while (returnCount < g_prefs->maxReproductions && stopSendingFlag == false)
    {
        ...
        if (UDTWrapper::CheckReceiver(server, 0, 1000) == 1)
        {
            uint32_t senderAddress;
            uint16_t senderPort;
            UDTSOCKET connection = UDTWrapper::StartAcceptor(server, &senderAddress, &senderPort);
            if (connection == UDT::INVALID_SOCK)
            {
            ...
            if (UDTWrapper::CheckReceiver(connection, 5, 0)) // anything to receive?
            {
                // read the command from the sender
                numBytes = UDTWrapper::ReceiveData(connection, requestMessage.text, sizeof(RequestMessage), 10, 0);
                ...
                if (strcmp(requestMessage.text, "req_xml") == 0)
                {
                    ...
                    dataMessagePtr->senderIP = senderAddress;
                    dataMessagePtr->senderPort = senderPort;
                    ...
                    numBytes = UDTWrapper::SendData(connection, dataMessage.data(), int(dataMessage.size()));
        ...
        } // UDT connection goes out of scope
    }
