...
int UDP::StartListener(uint16_t port)
{
    ...
    setsockopt(m_RecSockfd, SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<char *>(&tv), sizeof(struct timeval));

    tv.tv_sec = long(m_sendTimeout / 1000000);
    tv.tv_usec = long(m_sendTimeout % 1000000);
    setsockopt(m_RecSockfd, SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<char *>(&tv), sizeof(struct timeval));
    ...
    if (port != 0)
    {
        if (bind(m_RecSockfd, reinterpret_cast<struct sockaddr *>(&m_myAddress), sizeof(struct sockaddr)) == -1)
        {
#if defined(_WIN32) || defined(WIN32)
    ...
    else
    {
        ...
        while (true)
        {
            port++;
            m_myAddress.sin_port = htons(port); // short, network byte order
            if (bind(m_RecSockfd, reinterpret_cast<struct sockaddr *>(&m_myAddress), sizeof(struct sockaddr)) == 0) break;
