...
Genome * Population::ChooseParent(int *parentRank)
{
    ...
    {
        *parentRank = g_random->GammaBiasedRandomInt(0, m_Population.size() - 1, g_prefs->gamma);
    ...
    {
        *parentRank = g_random->RandomInt(0, m_Population.size() - 1);
        return m_Population[*parentRank];
    }
    ...
    {
        *parentRank = g_random->RankBiasedRandomInt(1, m_Population.size()) - 1;
        return m_Population[*parentRank];
    }
    ...
    {
        *parentRank = g_random->SqrtBiasedRandomInt(0, m_Population.size() - 1);
        return m_Population[*parentRank];
    }
...
int Population::InsertGenome(Genome *genome, int targetPopulationSize)
{
    ...
    {
        // std::cerr << "InsertGenome not unique; m_Population.size() = " << m_Population.size() << "\n";
        delete genome;
    ...
    if (m_Population.size() > targetPopulationSize)
    {
        ...
        if (found)
        {
            ...
            // std::cerr << "InsertGenome deleting from m_Population; size = " << m_Population.size() << "\n";
        }
        else // shouldn't actually ever get her but it's there just in case
        {
            ...
            // std::cerr << "InsertGenome deleting from m_Population (Logic Error); size = " << m_Population.size() << "\n";
        }
...
{
    if (nBest <= 0 || nBest > int(m_Population.size())) nBest = m_Population.size();
