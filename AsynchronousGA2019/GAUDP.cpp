...
int GAMain::Evolve(Population *startPopulation, Population *thePopulation, std::ostream *outFile, const std::string &dirname)
{
    ...
    // start the UDP listener/talker
    ThreadedUDP udp;
    if (m_logLevel > 2) udp.setDebug(true);
//    if (udp.StartTalker() != 0)
//    {
//        ReportProgress("Error setting up talker"s, 0);
//        return 1;
//    }
    if (udp.StartListener(g_prefs->tcpPort) != 0)
    {
        ReportProgress("Error setting up receiver - port probably in use"s, 0);
        return 1;
    }

    ReportProgress(ToString("UDP port %d open", g_prefs->tcpPort), 1);
    ...
    int lossCount = 0;
    while (returnCount < g_prefs->maxReproductions)
    {
        ...
        }        // first check to see if there is data coming in
        std::shared_ptr<ThreadedUDP::Packet> threadedPacket = udp.GetNextPacket();
        if (!threadedPacket) continue;
        if (m_logLevel)
        {
            bool networkOrder = true;
            ip = ConvertAddressPortToString(threadedPacket->address.sin_addr.s_addr, threadedPacket->address.sin_port, networkOrder);
        ...
        // request for XML
        if (reinterpret_cast<udp::RequestItemUDPPacket *>(threadedPacket->data.data())->type == udp::request_item &&
                reinterpret_cast<udp::RequestItemUDPPacket *>(threadedPacket->data.data())->item == udp::xml)
        {
            dataMessage.resize(sizeof(DataMessage) + m_baseXMLFile.GetSize() * sizeof(char));
            DataMessage *dataMessagePtr = reinterpret_cast<DataMessage *>(dataMessage.data());
            std::strcpy(dataMessagePtr->text, "xml");
            ...
            if (reinterpret_cast<udp::RequestItemUDPPacket *>(threadedPacket->data.data())->redundancy == 0)
                numBytes = udp.SendText(reinterpret_cast<udp::RequestItemUDPPacket *>(threadedPacket->data.data())->packetID,
                                        threadedPacket->address, dataMessage);
            else
                numBytes = udp.SendFEC(reinterpret_cast<udp::RequestItemUDPPacket *>(threadedPacket->data.data())->packetID,
                                       threadedPacket->address, dataMessage, reinterpret_cast<udp::RequestItemUDPPacket *>(threadedPacket->data.data())->redundancy);
        ...
        // request for a genome
        if (reinterpret_cast<udp::RequestItemUDPPacket *>(threadedPacket->data.data())->type == udp::request_item &&
                reinterpret_cast<udp::RequestItemUDPPacket *>(threadedPacket->data.data())->item == udp::genome)
        {
            ...
            {
                // one problem is that because the communications are lossy, we might not actually get all
                // of the start population rescored and that might be sub-optimal
            ...
            dataMessage.resize(sizeof(DataMessage) + startPopulation->GetFirstGenome()->GetGenomeLength() * sizeof(double));
            DataMessage *dataMessagePtr = reinterpret_cast<DataMessage *>(dataMessage.data());
            std::strcpy(dataMessagePtr->text, "genome");
            dataMessagePtr->runID = submitCount;
            dataMessagePtr->genomeLength = startPopulation->GetFirstGenome()->GetGenomeLength();
            dataMessagePtr->xmlLength = uint32_t(m_baseXMLFile.GetSize());
            std::copy(std::begin(m_md5), std::end(m_md5), std::begin(dataMessagePtr->md5));
            ...
            if (reinterpret_cast<udp::RequestItemUDPPacket *>(threadedPacket->data.data())->redundancy == 0)
                numBytes = udp.SendText(reinterpret_cast<udp::RequestItemUDPPacket *>(threadedPacket->data.data())->packetID, threadedPacket->address, dataMessage);
            else
                numBytes = udp.SendFEC(reinterpret_cast<udp::RequestItemUDPPacket *>(threadedPacket->data.data())->packetID, threadedPacket->address, dataMessage, reinterpret_cast<udp::RequestItemUDPPacket *>(threadedPacket->data.data())->redundancy);
            if (numBytes != int(dataMessage.size()))
            {
            ...
            runSpecifier->sender = threadedPacket->address;
        ...
        if (reinterpret_cast<udp::RequestItemUDPPacket *>(threadedPacket->data.data())->type == udp::request_item &&
                reinterpret_cast<udp::RequestItemUDPPacket *>(threadedPacket->data.data())->item == udp::score)
        {
#ifdef USE_QT_AGA
            if (returnCount % 100 == 0) QMetaObject::invokeMethod(m_callingWidget, "setResultNumber", Qt::QueuedConnection, Q_ARG(int, returnCount));
#endif
            int index = reinterpret_cast<udp::RequestItemUDPPacket *>(threadedPacket->data.data())->runID;
            double result = reinterpret_cast<udp::RequestItemUDPPacket *>(threadedPacket->data.data())->score;
            ...
            if (iter->second->sender.sin_addr.s_addr != threadedPacket->address.sin_addr.s_addr ||
                    iter->second->sender.sin_port != threadedPacket->address.sin_port)
            {
                ReportProgress(ToString("Sample %d found but from wrong client", index, result, ip.c_str()), 1);
                continue;
            }
            ...
            if (returnCount % g_prefs->outputStatsEvery == g_prefs->outputStatsEvery - 1)
            {
                ...
                *outFile << tenPercentiles << "\n";
                outFile->flush();
                ReportProgress(ToString("Return count = %d Results lost = %d", returnCount, lossCount), 1);
            }

            if (returnCount % g_prefs->saveBestEvery == g_prefs->saveBestEvery - 1)
            {
                if (thePopulation->GetLastGenome()->GetFitness() > maxFitness)
                {
                    ...
                    ReportProgress(ToString("Best score is now %g", maxFitness), 1);
#ifdef USE_QT_AGA
            ...
            if (returnCount % g_prefs->savePopEvery == g_prefs->savePopEvery - 1)
            {
        ...
        ReportProgress(ToString("Unrecognised message: %d", int(reinterpret_cast<udp::RequestItemUDPPacket *>(threadedPacket->data.data())->type)), 0);
    }
    ...
    ReportProgress(ToString("Return count = %d Results lost = %d", returnCount, lossCount), 0);
    ReportProgress(ToString("Best score is %g", maxFitness), 0);
    return 0;
}
...
std::string GAMain::ConvertAddressPortToString(uint32_t address, uint16_t port, bool networkOrder)
{
    if (networkOrder)
    {
...
std::string GAMain::ConvertAddressToString(uint32_t address, bool networkOrder)
{
    if (networkOrder)
    {
        address = ntohl(address);
